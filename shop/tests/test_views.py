...
class ProductListViewTest(TestCase):
    ...
    def test_view_context_contains_products(self):
        ...
        self.assertEqual(len(response.context['products']), 10)  # Oczekiwana liczba produktów na stronie
    ...
    def test_view_pagination_second_page(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context['products']), 5)  # Pozostałe produkty na drugiej stronie

    def test_filter_by_category(self):
        response = self.client.get(reverse('shop:product_list_by_category', args=['test-category']))
        self.assertEqual(response.status_code, 200)
        self.assertTrue(all(product.category == self.category for product in response.context['products']))

    def test_context_contains_categories(self):
        response = self.client.get(reverse('shop:product_list'))
        self.assertIn('categories', response.context)
        self.assertEqual(response.context['categories'].count(), 1)  # Sprawdzenie liczby kategorii w kontekście
...
class ProductDetailViewTest(TestCase):
    ...
    def test_view_url_exists_at_desired_location(self):
        response = self.client.get(reverse('shop:product_detail', args=[self.product.pk, self.product.slug]))
        self.assertEqual(response.status_code, 200)
